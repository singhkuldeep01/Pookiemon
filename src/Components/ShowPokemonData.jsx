  ...
  return(
    <>
        <div className='flex mt-16 gap-8 text-black items-center'>
            ...
            <div className='w-max h-max  flex flex-col justify-center items-center px-10 text-white bg-zinc-900 py-6 min-w-[500px]'>
                ...
                <div className=' rounded-lg p-2 mt-2'>
                    ...
                    <div className='flex gap-2 justify-center items-center text-lg'>
                        Abilities : {data.abilities?.map((item , index) => {
                            return <div className=' p-1 rounded-full text-white px-4 bg-red-500' key={index}>{item.ability.name}</div>
                        })}
                    </div>
                </div>
              ...
              return <div className='p-1 rounded-full text-white px-4 bg-green-800' key={item.move.name}>{item.move.name}</div>;
            })}
                </div>
                <div className=' p-2 rounded-sm bg-purple-600 w-full mt-4 text-lg'>Types: {data.types?.[0]?.type?.name }</div>
            </div>
        </div>
        <button className='text-white bg-blue-500 px-5 py-2 text-lg ml-2 mt-8 hover:bg-blue-600 duration-75 rounded-full shadow-xl' onClick={onCatch}>Catch</button>
    </>
  );
}
